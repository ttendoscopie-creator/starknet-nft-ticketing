...
const TX_TIMEOUT_MS = 120_000; // Increased from 60s to 120s for sequencer delays

// --- u256 encoding helper (SECURITY FIX MED-15: proper 128-bit mask) ---

const U128_MASK = (1n << 128n) - 1n;

function encodeU256(value: bigint): { low: bigint; high: bigint } {
  return { low: value & U128_MASK, high: value >> 128n };
}

// --- Nonce Mutex (CRIT-04 fix) ---
...
const nonceMutex = new NonceMutex();

// Serialize on-chain writes through the nonce mutex
async function executeWithNonce<T>(fn: () => Promise<T>): Promise<T> {
  await nonceMutex.acquire();
  try {
    return await fn();
  } finally {
    nonceMutex.release();
...
function isTransientError(err: unknown): boolean {
  ...
  return (
    ...
    msg.includes("nonce") || // Nonce errors are transient with mutex
    msg.includes("429") ||
    msg.includes("503") ||
    msg.includes("502")
  );
...
async function withRetry<T>(fn: () => Promise<T>): Promise<T> {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      // Only count transient errors toward circuit breaker (SECURITY FIX)
  ...
  return withRetry(async () => {
    ...
    for (const event of transferEvents) {
      ...
      if (event.data && event.data.length >= 4) {
        ...
        // SECURITY FIX (HIGH-14): Validate sender if provided
        const senderOk = !expectedSender || num.toHex(from) === num.toHex(expectedSender);
